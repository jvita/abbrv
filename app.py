    ...
    for li, line in enumerate(lines):
        ...
        for word in word_splines:
            for points in word:
                ...
                highest_point = max(points[:, 1].max(), highest_point)
                lowest_point = min(points[:, 1].min(), lowest_point)
        ...
        if li > 0:
            current_vertical_offset += abs(highest_point)
